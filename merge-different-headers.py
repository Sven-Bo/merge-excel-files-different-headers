...
with xw.App(visible=False) as app:
    for file in files:
        ...
        # convert table to dataframe, rename headers, add source column & append to dataframes list
    ...
    wb_combined.sheets[0].range("A1").options(pd.DataFrame, index=False).value = combined_df
    wb_combined.save(Path(__file__).parent / "Combined_Data.xlsx")
    wb_combined.close()